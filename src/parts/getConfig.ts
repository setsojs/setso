...
export async function getConfigFromFiles(): Promise<z.infer<typeof optionsSchema> | undefined> {
    const configFile = await getFile()
    if (configFile === undefined){
        return undefined;
    }
    const config: z.infer<typeof optionsSchema> = await json5.parse((await readFile(configFile)).toString())
    return config;
}    
